...
def gen_all_pic():
    ...
    while index < gen_count:
        log.info(
            "-" * 20 + " generate new picture {index}/{gen_count}".format(index=index, gen_count=gen_count) + "-" * 20
        )
        dump_data = gen_pic()
        # Write label
        if dump_data:
...
def _gen_coco(layout_data, json_path):
    ...
    with open(json_path, "w") as f:
        ...
        for fragment in fragment_list:
            ...
            kv_lower = {ascii_lowercase[idx]: idx + base_offset + len(kv_digits) + len(kv_upper) for idx in
